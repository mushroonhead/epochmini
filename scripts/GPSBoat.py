...
def talker():
	...
	while not rospy.is_shutdown():
		...
		##print data
		##print len(data)
		if data[0] == 'Z' and len(data) >= 32 and len(data) < 35:
			...
			try:
				...
				poseGPS.pose.position.x = ne[1]
				poseGPS.pose.position.y = ne[0]
				...
				##rospy.loginfo(log)
				print "%f %f %f %f" % (ne[1], ne[0], float_list[2], float_list[3])
			except: continue ##print "Data Error! Data :  %s" % data
		elif data[0] == "Y":
			continue
		else:
			continue ##print "Data Error or Message: %s" % data
		rate.sleep()
